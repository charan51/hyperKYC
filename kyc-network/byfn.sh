...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/citiBank.kyc.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/sbi.kyc.com/ca && ls *_sk)
  fi
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.kyc.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.kyc.com
    docker cp -a orderer.kyc.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.kyc.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.kyc.com

    for PEER in peer0.citiBank.kyc.com peer1.citiBank.kyc.com peer0.sbi.kyc.com peer1.sbi.kyc.com; do
...
function networkDown() {
  # stop org3 containers also in addition to citiBank and org2, in case we were running sample to add org3
...
# and two Peer Orgs (``citiBank`` & ``Org2``) each managing and maintaining two peer nodes.
...
# (``peer0.citiBank.kyc.com`` & ``peer0.sbi.kyc.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for CitiBankMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/CitiBankMSPanchors.tx -channelID $CHANNEL_NAME -asOrg CitiBankMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for CitiBankMSP..."
    exit 1
  fi
  ...
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/SbiMSPanchors.tx -channelID $CHANNEL_NAME -asOrg SbiMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
